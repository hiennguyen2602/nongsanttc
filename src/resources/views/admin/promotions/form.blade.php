<div class="x_panel">
    ...
    <div class="x_content">
        <form method="POST" action="{{ isset($promotion) ? route('admin.promotions.update', $promotion) : route('admin.promotions.store') }}" class="max-w-xl">
            ...
            <div class="mb-3"><label class="form-label">Đơn tối thiểu</label><input type="number" name="min_order" value="{{ old('min_order', $promotion->min_order ?? 0) }}" class="form-control"></div>
            <div class="mb-3"><label class="form-label">Số tiền giảm</label><input type="number" name="discount_amount" value="{{ old('discount_amount', $promotion->discount_amount ?? 0) }}" class="form-control"></div>
