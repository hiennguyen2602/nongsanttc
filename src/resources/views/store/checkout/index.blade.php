...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
        ...
        @if ($errors->any())
            <div class="mb-4 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{{ $errors->first() }}</div>
        @endif
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <form method="POST" action="{{ route('checkout.store') }}" class="space-y-4 rounded-lg border border-slate-200 p-6">
                    @csrf
                    <div><label class="mb-1 block text-sm font-medium">Họ tên *</label><input name="name" value="{{ old('name') }}" required class="w-full rounded border-slate-300 text-sm"></div>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div><label class="mb-1 block text-sm font-medium">Số điện thoại *</label><input name="phone" value="{{ old('phone') }}" required class="w-full rounded border-slate-300 text-sm"></div>
                        <div><label class="mb-1 block text-sm font-medium">Email</label><input type="email" name="email" value="{{ old('email') }}" class="w-full rounded border-slate-300 text-sm"></div>
                    </div>
                    <div><label class="mb-1 block text-sm font-medium">Địa chỉ giao hàng *</label><textarea name="address" rows="3" required class="w-full rounded border-slate-300 text-sm">{{ old('address') }}</textarea></div>
                    <div><label class="mb-1 block text-sm font-medium">Ghi chú</label><textarea name="note" rows="2" class="w-full rounded border-slate-300 text-sm">{{ old('note') }}</textarea></div>
            ...
            <div class="space-y-4">
                ...
                <form method="POST" action="{{ route('checkout.promo') }}" class="flex gap-2">
                    @csrf
                    <input type="text" name="promo_code" value="{{ $promoCode }}" placeholder="Mã khuyến mãi" class="flex-1 rounded border-slate-300 text-sm uppercase">
